...
class JapanAnimeMigrator(MiniMigrator):
    @staticmethod
    def insert_subject(conn, cursor):
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS subject(
        ...
        with open("./input/subject.jsonlines", 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if item['platform'] == 2 and "日本" in item['meta_tags']:
                    cursor.execute('''
                        INSERT INTO subject(id, name, name_cn, infobox, platform, summary, nsfw, 
                        meta_tags, date, rating, status) VALUES (?,?,?,?,?,?,?,?,?,?,?)''', (
